...
public class LoginGUI extends JFrame {
	...
	public LoginGUI() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fld_userTc.getText().length() == 0 || fld_userPass.getText().length() == 0) {
					...
					try {
						...
						while (rs.next()) {
							if (fld_userTc.getText().equals(rs.getString("tcno")) && fld_userPass.getText().equals(rs.getString("password"))) {
								...
								MemberGUI membGUI = new MemberGUI();
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fld_adminId.getText().length() == 0 || fld_adminPass.getText().length() == 0) {
					...
					try {
						...
						while (rs.next()) {
							if (fld_adminId.getText().equals(rs.getString("tcno")) && fld_adminPass.getText().equals(rs.getString("password"))) {
		...
		JLabel lbl_ForgottenAdmin = new JLabel("Giris bilgilerinizi hatirlayamiyorsaniz lutfen ilgili birimle iletisime geciniz");
